...
subtest 'small server2client frame' => sub {
    ...
    my $bin = $create_builder->(0)->final(1)->send($payload);
    ...
    is_bin($create_builder->(0)->final(1)->send_av([qw/pr ev ed/]), $bin, "it mode ok");
};
...
subtest 'small client2server frame' => sub {
    my $builder = $create_builder->(1, {opcode => OPCODE_TEXT, final => 1});
    my $payload = "preved"; # must be <= 125
...
subtest 'empty frame still masked' => sub {
    my $builder = $create_builder->(1,{opcode => OPCODE_BINARY, final => 1});
    my $bin = $builder->send("");
