...
for concrete child classes L<Protocol::WebSocket::XS::ClientParser> and
...
butis slow and reduces compression ratio; C<mem_level=9> uses maximum
...
successfully parsed server responce and on B<server_parser> side the
...
Optionally appends $data to internal buffer and tries to parse it.
...
Max payload length is 125, everythin bigger will be cut.
...
Max payload length is 123, everythin bigger will be cut.

The constructed frame is always final and is never compressed.

=head2 send_message($settings)
...
=head2 send_message_multiframe($settings)
...
This method sends one mesassage as multiple frames. It iss useful if it is
