...
=head2 accept_response()

=head2 accept_response(Protocol::WebSocket::XS::ConnectResponse $object)

Returns serialized B<positive> responce from server to client.
...
C<$object> with predefined headers and/or custom extensions.

=head2 accept_error()

=head2 accept_error(Protocol::WebSocket::XS::ConnectResponse $object)

Returns serialized B<negative> responce from server to client. Further
