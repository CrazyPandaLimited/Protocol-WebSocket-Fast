...
Protocol::WebSocket::XS - WebSocket extremely fast websocket protocol
...
=head1 SYNOPSIS
    ...
    my $data_6 = $client->send_message_av({deflate => 0, payload => \@payload_pieces});
...
C<Protocol::WebSocket::XS> is extremely fast server/client zero-copy
incremental WebSocket (L<rfc6455|https://tools.ietf.org/html/rfc6455>) parser
and serialization library with Perl and C++ interface.
...
The C++ interfaces of the library can be used to maximize performance, due to
no overhead occurs in Perl-C++ data marshalling; in other words it is friendly
to XS servers and frameworks, written in C++.

The <Protocol::WebSocket::XS> is B<not> drop-in replacement of <Protocol::WebSocket>,
as it uses sligthly different API.

=head1 BENCHMARKS (Protocol::WebSocket vs Protocol::WebSocket::XS)

(see misc/bench.plx for full code)
    ...
    Protocol::WebSocket :  1 wallclock secs ( 1.08 usr +  0.00 sys =  1.08 CPU) @ 113777.78/s (n=122880)
    Protocol::WebSocket::XS:  2 wallclock secs ( 1.08 usr +  0.00 sys =  1.08 CPU) @ 2316928.70/s (n=2502283)


=head1 EXPORTED CONSTANTS

=head2 OPCODE_CONTINUE
...
=head2 CLOSE_PROTOCOL_ERROR

=head2 CLOSE_INVALID_DATA

=head2 CLOSE_UNKNOWN

=head2 CLOSE_ABNORMALLY

=head2 CLOSE_INVALID_TEXT

=head2 CLOSE_BAD_REQUEST

=head2 CLOSE_MAX_SIZE

=head2 CLOSE_EXTENSION_NEEDED

=head2 CLOSE_INTERNAL_ERROR
